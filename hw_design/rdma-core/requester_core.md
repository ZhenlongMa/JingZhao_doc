...
ReqTransCore负责处理本地通信操作的发起部分。该模块由四个流水级组成：

- 第一个流水级负责接收subWQE，并发起上下文读请求；
- 第二个流水级接收到上下文读响应后发起Memory Region元数据的读请求；
- 第三个流水级收到Memory Region元数据响应后发起数据gather的读请求；
- 第四个流水级收集所有待发送数据，发送至transport引擎。

Thread 1功能较为简单，从SQ Management模块接收到sub-WQE后发起对应的队列上下文读请求。
