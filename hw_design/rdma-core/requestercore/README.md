...
| RX_RESP_fetch_mr_ingress_last | output | 1 | OoOStation(MRMgt) | RX_RESP内存区域请求结束标志 |
| RX_RESP_fetch_mr_ingress_ready | input | 1 | OoOStation(MRMgt) | RX_RESP内存区域请求就绪反压 |
| RX_RESP_fetch_mr_egress_valid | input | 1 | OoOStation(MRMgt) | RX_RESP内存区域响应有效 |
| RX_RESP_fetch_mr_egress_head | input | 160 | OoOStation(MRMgt) | RX_RESP内存区域响应头 |
| RX_RESP_fetch_mr_egress_data | input | 288 | OoOStation(MRMgt) | RX_RESP内存区域响应数据 |
| RX_RESP_fetch_mr_egress_start | input | 1 | OoOStation(MRMgt) | RX_RESP内存区域响应开始标志 |
| RX_RESP_fetch_mr_egress_last | input | 1 | OoOStation(MRMgt) | RX_RESP内存区域响应结束标志 |
| RX_RESP_fetch_mr_egress_ready | output | 1 | OoOStation(MRMgt) | RX_RESP内存区域响应就绪反压 |
| RX_RESP_cq_req_valid | output | 1 | CompletionQueueMgt | RX_RESP CQ请求有效 |
| RX_RESP_cq_req_head | output | 64 | CompletionQueueMgt | RX_RESP CQ请求头 |
| RX_RESP_cq_req_ready | input | 1 | CompletionQueueMgt | RX_RESP CQ请求就绪反压 |
| RX_RESP_cq_resp_valid | input | 1 | CompletionQueueMgt | RX_RESP CQ响应有效 |
| RX_RESP_cq_resp_head | input | 96 | CompletionQueueMgt | RX_RESP CQ响应头 |
| RX_RESP_cq_resp_ready | output | 1 | CompletionQueueMgt | RX_RESP CQ响应就绪反压 |
| RX_RESP_eq_req_valid | output | 1 | EventQueueMgt | RX_RESP EQ请求有效 |
| RX_RESP_eq_req_head | output | 64 | EventQueueMgt | RX_RESP EQ请求头 |
| RX_RESP_eq_req_ready | input | 1 | EventQueueMgt | RX_RESP EQ请求就绪反压 |
| RX_RESP_eq_resp_valid | input | 1 | EventQueueMgt | RX_RESP EQ响应有效 |
| RX_RESP_eq_resp_head | input | 96 | EventQueueMgt | RX_RESP EQ响应头 |
| RX_RESP_eq_resp_ready | output | 1 | EventQueueMgt | RX_RESP EQ响应就绪反压 |
| RX_RESP_scatter_req_wen | output | 1 | DMA引擎 | RX_RESP DMA写请求使能 |
| RX_RESP_scatter_req_din | output | 160 | DMA引擎 | RX_RESP DMA写请求数据(地址+长度) |
| RX_RESP_scatter_req_prog_full | input | 1 | DMA引擎 | RX_RESP DMA写请求FIFO满 |
| RX_RESP_scatter_data_wen | output | 1 | DMA引擎 | RX_RESP DMA写数据使能 |
| RX_RESP_scatter_data_din | output | 512 | DMA引擎 | RX_RESP DMA写数据 |
| RX_RESP_scatter_data_prog_full | input | 1 | DMA引擎 | RX_RESP DMA写数据FIFO满 |
| RX_RESP_delete_req_valid | output | 1 | PacketBuffer | RX_RESP删除包请求有效 |
| RX_RESP_delete_req_head | output | 24 | PacketBuffer | RX_RESP删除包请求头(队列索引+槽位) |
| RX_RESP_delete_req_ready | input | 1 | PacketBuffer | RX_RESP删除包请求就绪反压 |
| RX_RESP_delete_resp_valid | input | 1 | PacketBuffer | RX_RESP删除包响应有效 |
| RX_RESP_delete_resp_start | input | 1 | PacketBuffer | RX_RESP删除包响应开始标志 |
| RX_RESP_delete_resp_last | input | 1 | PacketBuffer | RX_RESP删除包响应结束标志 |
| RX_RESP_delete_resp_data | input | 512 | PacketBuffer | RX_RESP删除包响应数据 |
| RX_RESP_delete_resp_ready | output | 1 | PacketBuffer | RX_RESP删除包响应就绪反压 |
